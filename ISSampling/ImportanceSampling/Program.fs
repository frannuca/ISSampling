...
let main argv = 
    ...
    let computeq = MonteCarlo.computeQuantileOnIS(Loss_IS|>Seq.map(fun (_,a,b) -> a,b)|> Array.ofSeq,1e-4)
    let Nqs = 500
    let cls = [0 .. Nqs]
                |> Seq.map(fun n -> 
                                    let s = 0.99+(1.0-0.99)*float(n)/float(Nqs)
                                    s,computeq(s))
                |>Frame.ofRecords
    
    framec.SaveCsv("E:/SmpleChart_B.csv")
    framecIS.SaveCsv("E:/ISChart_B.csv")
    cls.SaveCsv("E:/quantiles2.csv")
